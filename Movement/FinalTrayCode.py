...
def run_bus(id_entered, angle_entered):
    ...
    while True:
        ...
        if angle.isdigit() and id.isdigit():
            angle = int(angle)
            id = int(id)
...
try:
    ...
    shoulder = pwm(name='shoulder', lower_limit=0, upper_limit=270, max_rotation_deg=270, dead_band=0.5, frequency=50, min_pulse_width=500, max_pulse_width=2500, pin=17)
    bus_servo = uart()
